...
class utils:
        ...
        def find_smip_equipment_of_name(self, relativename):
                smp_query = f''' query get_equipment {{
                  equipments(condition: {{relativeName: "{relativename}"}}) 
                  {{
                        id
                  }}
                }}'''
        ...
        def update_parent(self, parentid, equipmentid):
                smp_mutation = f'''
                  mutation update_parent {{
        ...
        def build_alias_clear_ts_mutation(self, ordinal, attributOrTagId):
                timestamp = self.make_datetime_utc()
                smp_mutation = f'''atr{ordinal}: replaceTimeSeriesRange(
                        input: {{
                        attributeOrTagId: "{attributOrTagId}"
                        startTime: "2023-06-14T04:00:00+00:00"
                        endTime: "{timestamp}"
                        }}
                        ) {{
                        json
